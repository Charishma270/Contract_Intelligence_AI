...
function Analyze() {
  ...
  const normalizeResults = (data) => {
    ...
    if (Array.isArray(data?.clauses)) return data.clauses;
    if (Array.isArray(data?.retrieved_chunks)) return data.retrieved_chunks;

    if (data?.answer || data?.response || data?.message) {
      return [
        {
          retrieved_label: "AI Analysis Result",
          clause_text: data.answer || data.response || data.message,
          explanation: data.explanation || "AI-generated analysis result.",
          risk_level: data.risk_level || data.risk || "Unknown",
          final_confidence: data.confidence || 0,
        },
      ];
    }
  ...
  const handleAnalyze = async () => {
    if (!input.trim()) {
      setError("Please enter a clause or legal query.");
      return;
    }
    ...
    try {
      ...
      if (normalized.length === 0) {
        setError("No analysis results returned from backend.");
      }
    } catch (err) {
    ...
    <Layout>
      <div className="space-y-8">
        ...
        <section className="rounded-3xl bg-white dark:bg-slate-900 p-7 shadow-sm border border-slate-100 dark:border-slate-800">
          <div className="mb-6 flex items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300">
              <FileSearch size={28} />
            </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
            <div className="relative flex-1">
              ...
              <textarea
                ...
                onChange={(e) => setInput(e.target.value)}
                rows={1}
                className="min-h-[58px] max-h-36 w-full resize-none rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-4 pl-12 pr-4 text-slate-900 dark:text-white outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                onInput={(e) => {
                  e.target.style.height = "auto";
                  e.target.style.height = `${Math.min(
                    e.target.scrollHeight,
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {[
              ...
              <button
                key={item}
                onClick={() => setInput(item)}
                className="rounded-full bg-blue-50 dark:bg-blue-900/40 px-4 py-2 text-sm font-semibold text-blue-600 dark:text-blue-300 transition hover:bg-blue-100"
              >
                {item}
              </button>
            ))}
        ...
        {error && (
          <section className="rounded-3xl border border-red-100 dark:border-red-800 bg-red-50 dark:bg-red-900/30 p-6 text-red-700 dark:text-red-300">
            <div className="flex items-center gap-3">
              <AlertCircle />
              <div>
                <h3 className="font-bold">Analysis Failed</h3>
        ...
        {loading && (
          <section className="rounded-3xl bg-white dark:bg-slate-900 p-10 text-center shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-3xl bg-blue-50 dark:bg-blue-900/40">
              <Loader2 className="animate-spin text-blue-600 dark:text-blue-300" size={38} />
            </div>
        ...
        {!loading && results.length === 0 && !error && (
          <section className="rounded-3xl bg-white dark:bg-slate-900 p-12 text-center shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-3xl bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300">
              <Brain size={38} />
            </div>
